...
if(isset($_GET['add'])) {
	...
	while($row = fetch_array($query)){
		...
		if($row['product_quantity'] != $_SESSION['product_' . $_GET['add']]){
			...
			set_message("Przepraszamy, mamy tylko " . $row['product_quantity'] . " " . $row['product_title'] . " dostępnych produktów");
			redirect("../checkout");
		}
...
function show_paypal_button(){
    ...
    <h4>Proszę się upewnić, czy podane dane adresowe na stronie paypal są prawidłowe</h4>
	
	<input type="image" name="upload"
    src="https://www.paypalobjects.com/webstatic/en_US/i/btn/png/silver-pill-paypal-60px.png" 
	alt="PayPal">
...
$item_quantity = 0;
	...
	$last_id = last_id(); //id ostatniej transakcji
	confirm($send_order);
